...
const WORKFLOW_STEPS = [
  {
    ...
    command: 'agy new project --name "Q3 Architecture"',
    output: "Project initialized.\nLoading workspace configuration...",
  },
  {
    id: "agent",
    label: "Read AGENT.md",
    command: "cat AGENT.md",
    output:
  ...
  },
  {
    id: "mcp",
    label: "Connect MCP",
    command: 'mcp connect --context "Q3 Architecture"',
    output: "Connected to Context Server.\nAccessing historical reports...",
  },
  {
    id: "git",
    label: "Git Checkout",
    command: "git checkout -b feature/q3-architecture",
    output: "Switched to a new branch 'feature/q3-architecture'",
  },
  {
    id: "code",
    label: "Write Code",
    command: 'agy execute --task "Implement new architecture"',
    output:
  ...
  },
  {
    id: "submission",
    label: "Submission",
    command: "git push origin feature/q3-architecture && pr create",
    output:
      ...
      <div className="terminal-workflow__container" ref={containerRef}>
        ...
        <div className="terminal-workflow__sidebar">
          {WORKFLOW_STEPS.map((step, index) => (
            ...
            >
              ...
              <span className="terminal-workflow__sidebar-label">
                {step.label}
              </span>
            </div>
          ))}
        ...
        <div className="terminal-workflow__window">
          <div className="terminal-workflow__window-header">
            ...
            <div className="terminal-workflow__window-title">
              workspace ~ craftly-robot
            </div>
          </div>
          ...
          <div className="terminal-workflow__window-body" ref={bodyRef}>
            <div className="terminal-workflow__history">
              {WORKFLOW_STEPS.slice(0, currentStep).map((step, index) => (
                <div
                  key={`history-${index}`}
                  className="terminal-workflow__history-item"
                >
                  <div className="terminal-workflow__line">
                    <span className="terminal-workflow__prompt">$</span>
                    <span className="terminal-workflow__command">
                      {step.command}
                    </span>
                  </div>
            ...
            {!isTyping &&
              displayedCommand === WORKFLOW_STEPS[currentStep]?.command && (
